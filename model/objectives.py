...
def compute_sdm(image_fetures, text_fetures, pid, logit_scale, image_id=None, factor=0.3, epsilon=1e-8):
    ...
    pid_dist = pid - pid.t() # compute the distance between pid
    ...
    # the cosine similarity as logits means the similarity between image and text
    ...
    # compute the loss, i2t_pred and t2i_pred are the predicted distribution
    i2t_pred = F.softmax(image_proj_text, dim=1)
    i2t_loss = i2t_pred * (F.log_softmax(image_proj_text, dim=1) - torch.log(labels_distribute + epsilon))
    t2i_pred = F.softmax(text_proj_image, dim=1)
    t2i_loss = t2i_pred * (F.log_softmax(text_proj_image, dim=1) - torch.log(labels_distribute + epsilon))
    # loss can be computed by the sum of i2t_loss and t2i_loss
