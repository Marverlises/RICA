...
class Contrastive(nn.Module):
    ...
    def compute_contrastive_loss(self, scores):
        ...
        # caption retrieval
        cost_s = (self.margin + scores - d1).clamp(min=0)  # .clamp(min=0) is the same as max(0, x)
        # compare every diagonal score to scores in its row
        # image retrieval
        cost_im = (self.margin + scores - d2).clamp(min=0)

        # clear diagonals
        mask = torch.eye(scores.size(
            0)) > .5
        I = mask
        if torch.cuda.is_available():
            I = I.cuda()
        cost_s = cost_s.masked_fill_(I, 0)  # masked_fill_() fills the elements of the tensor with the value where the mask is True
...
class AlignmentContrastiveLoss(Contrastive):
    ...
    def forward(self, im_set, s_seq, im_len, s_len):
        ...
        im_set = im_set[:, 1:, :]
        # copy a new tensor to store the result, the dim is (s_seq.shape(0), s_seq.shape(1) - 1, s_seq.shape(2))
        cap_seq = torch.zeros(s_seq.size(0), s_seq.size(1) - 1, s_seq.size(2), device=s_seq.device)
        # remove the cls token of the features: at s_len, and concat the remaining all the features
        for i in range(len(s_len)):
            cap_seq[i, :, :] = torch.cat((s_seq[i, :s_len[i] - 1, :], s_seq[i, s_len[i]:, :]), dim=0)
...
class ContrastiveLoss(Contrastive):
    ...
    def forward(self, im, s):
        ...
        # find the most similar image and caption in order to find the hardest corresponding caption and image
        im_scores = self.sim(im, im)
        s_scores = self.sim(s, s)

        # intra_loss = self.mse(im_scores,s_scores)
        intra_loss = ((s_scores - im_scores).abs().max(0)[0] - self.sigma).clamp(min=0).sum()
