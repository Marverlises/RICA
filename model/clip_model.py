...
logger = logging.getLogger("RICA.model")
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: Tuple[int, int], patch_size: int, stride_size: int, width: int, layers: int,
        ...
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=stride_size,
                               bias=False)
        # scale is initialized in the same way as the official JAX impl
    ...
    def forward(self, x: torch.Tensor):
        ...
        # the following code aims to add the class embedding and the positional embedding to the image features
        x = torch.cat(
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        ...
        img_lengths = torch.tensor([image_features.shape[1] for i in range(image_features.shape[0])])
        return image_features, img_lengths
    ...
    def encode_text(self, text):
        ...
        # compute each caption's length(start from sot_token and end at eot_token)
        text_lengths = torch.tensor(
            [torch.where(text[i] == eot_token)[0][0] - torch.where(text[i] == sot_token)[0][0] + 1 for i in
             range(text.shape[0])])
        return text_features, text_lengths
    ...
    def forward(self, image, text):
        image_features, img_lengths = self.encode_image(image)
        text_features, text_lengths = self.encode_text(text)
        ...
        return image_features, text_features, text_lengths, img_lengths
    ...
    def load_param(self, state_dict):
        ...
        for k, v in param_dict.items():
            if k == 'visual.positional_embedding' and v.shape != self.visual.positional_embedding.shape:
                ...
                v = resize_pos_embed(v, self.positional_embedding, 1, self.context_length)
            try:
...
def convert_weights(model: nn.Module):
    ...
    this method aims to convert the model weights to fp16
    """
...
def build_CLIP_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int,
                                      jit: bool = False, download_root: str = None, CLIP_alignment_args: dict = None):
    """Load a CLIP model
    ...
    model = CLIP(**model_cfg, args=CLIP_alignment_args)
